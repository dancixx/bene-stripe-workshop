...
app.post('/api/stripe-init', async (req, res) => {
  const stripe = require('stripe')(
    'sk_test_51HmUmxK3wzCIekEvm7M26tRN5EuvHsQZJyomwd4E5loEF5Mk6S1bCwOQsjga6WPhXEWBg0onreLzgqKm3bdqVJFv00911E1i3p' //your key here
  );

  const product = [...elegantProducts, ...sportyProducts].find(
    (p) => p.id === req.body.id
  );
